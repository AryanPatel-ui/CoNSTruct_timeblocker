...
const getOidcConfig = memoize(
  async () => {
    if (!process.env.REPL_ID) {
      throw new Error(
        "REPL_ID environment variable is required for Replit Auth. This is automatically set in Replit environments."
      );
    }
...
export async function setupAuth(app: Express) {
  ...
  app.get("/api/logout", (req, res) => {
    if (!process.env.REPL_ID) {
      return res.status(500).json({ message: "REPL_ID not configured" });
    }
    req.logout(() => {
      res.redirect(
